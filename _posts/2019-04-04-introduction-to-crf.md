...
LR模型是分类任务中的判别模型，直接使用Ligistic函数**建模条件概率P(y|x)**。 
实际上，logistic函数是softmax的特殊形式，并且**LR等价于最大熵模型**，完全可以写成最大熵的形式：
...
P(l_i=y|l_{i-1} = x)
$$，要为CRF定义如下样子的特征函数
$$f_{xy}( s, i, l_i, l_{i-1}) = 1
$$ 如果$$l_i =y   $$且$$l_{i-1} = x$$ ,并且为每个特征函数赋如下的权重 $$w_{xy} = log P(l_i = y|l_{i_1} = x)$$



- 对于HMM发射概率矩阵 $$P(w_i=z|l_i=x)$$，对应定义CRF的"发射特殊函数"，形如$$g_{xy}(s,i,l_i,l_{i-1}) = 1$$， 如果$$w_i=z$$ 并且
$$l_i = x$$。与之对应的每个特征函数的权重$$w_{x,z} = log P(w_i=z|l_i=x)$$
